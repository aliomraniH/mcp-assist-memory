...
async def test_same_event_id_applied_once(backend, ns):
    # FLIPPED by v3 item 4: an identical retry is a visible no-op; a different
    # payload on the same event_id no longer silently echoes — it errors with
    # idempotency_conflict (covered in test_idempotency_fingerprint).
    eid = str(uuid.uuid4())
    first = await backend.memory_save(ns, "k", "v1", event_id=eid)
    second = await backend.memory_save(ns, "k", "v1", event_id=eid)
