...
**Resolved fragility:** the test used to *hardcode* posting the default admin-password
literal on the login step while only `setdefault`-ing it. Because `setdefault` is a no-op
when the var is already present, any environment that injects a *different* `ADMIN_PASSWORD`
— this Replit dev container's real secret AND the value CI sets in the test workflow — made
the login return 200 ("Incorrect") instead of 303, so the test failed (CI was red on it).
...
the CI value. The wrong-password probe still posts a deliberately-wrong literal.
